...
$$
\log p(\mathbf{x})=\sum_{i=1}^{d}\log p(x_i|\mathbf{x_{1:i-1}}) \quad\mathbf{x}为一个样本的字母序列
$$
...
- 例：上图$p(x_3|x_2)$被标记为2，只与前一层被标记为1的层连接，依次溯源的输入，这一项只与$x_2$有关，而与$x_1,x_3$无关。同理$p(x_1|x_2,x_3)$溯源到输入只与$x_2,x_3$有关
- 模型代码
